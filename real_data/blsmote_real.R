...
for (foldnum.test in 1:n_fold) {
  ...
  for (foldnum.tune in 1:n_fold) {
    ...
    ################################################
      ...
      tunetrain_tunevalid_split_now_train_dbsmote <- get.BLSMOTE.oversample(tunetrain_tunevalid_split_now$train, OR)
  ...
  train_test_split_now$train <- get.BLSMOTE.oversample( train_test_split_now$train,  OR)
