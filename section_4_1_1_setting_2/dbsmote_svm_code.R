...
library(e1071)
library(smotefamily)
source( paste0(utildir, "/shifted_tri_waveform_ver2.R") ) #setting 2 of section 4.1.1
...
for (foldnum.test in 1:n_fold) {
  ...
  for (foldnum.tune in 1:n_fold) {
    ...
    for (k in 1:length(param.set.OR)){
      tunetrain_tunevalid_split_now_train_dbsmote <- get.DBSMOTE.oversample(tunetrain_tunevalid_split_now$train, param.set.OR[k])
  ...
  train_test_split_now$train <- get.DBSMOTE.oversample( train_test_split_now$train,  OR_best)
