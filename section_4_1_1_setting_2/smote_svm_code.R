...
for (foldnum.test in 1:n_fold) {
  ...
  for (foldnum.tune in 1:n_fold) {
    ...
    for (k in 1:length(param.set.OR)){
      tunetrain_tunevalid_split_now_train_dbsmote <- get.SMOTE.oversample(tunetrain_tunevalid_split_now$train, param.set.OR[k])
  ...
  train_test_split_now$train <- get.SMOTE.oversample( train_test_split_now$train,  OR_best)
